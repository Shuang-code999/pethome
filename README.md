...
> 测试条件：voucher=6，1000 个不同用户，1000 并发，JVM 预热后直连后端（Tomcat 默认 200 线程）
...
| **QPS** | **312** |
| 平均延迟 | 1737.8ms |
| p50 / p95 / p99 | 1745 / 2610 / 2802 ms |

> 调优后（Tomcat 线程池 max=1000）QPS 提升至 **468**（+50%），瓶颈从线程池排队转移到 Redis 单线程 Lua 执行。

### 关单 vs 支付竞争验证

> 测试条件：50 个订单，支付超时 5s，并发支付 + 超时关单
...
| 双重处理（同一订单既支付又关单） | 0 |
| 库存超回滚 | 0 |
| DB 最终状态 | 50 条全部 status=1（已支付） |
| Redis 库存 | 150→100（消耗 50，无回滚） |

> 结论：`payIfUnpaid` + `cancelIfUnpaid` 乐观锁并发竞争 **0 错乱**，最终一致性验证通过。

### 多级缓存命中率

> 测试条件：2000 次商品详情请求（热点 ID + 长尾 ID + 不存在 ID 混合），200 并发
...
> 测试条件：令牌桶 tokens=10/s, capacity=20

| 指标 | 单 IP（Mode A） | 5 IP 隔离（Mode B） |
|------|----------------|-------------------|
| 总请求 | 100 | 250 |
| 放行（200） | 46 | 111 |
| 限流（429） | 54 | 139 |

> 结论：单 IP 超限返回 429，多 IP 之间令牌桶隔离互不影响，IP+URI 维度限流验证通过。
...
- **缓存效果显著**：L1 Caffeine 命中率 90.8%，叠加空值缓存后总命中率 95.8%，DB 查询量下降 95.8%。
